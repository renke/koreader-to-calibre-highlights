...
def koreader_highlight_to_calibre(koreader_highlight, calibre_book_container):
    if "pos0" not in koreader_highlight:
        print("⚠️ No start position in highlight", koreader_highlight)

        return None

    if "pos1" not in koreader_highlight:
        print("⚠️ No end position in highlight", koreader_highlight)

        return None

    (calibre_start_cfi, start_spine_index, start_spine_name) = (
        koreader_pos_to_calibre_cfi(koreader_highlight["pos0"], calibre_book_container)
    )

    (calibre_end_cfi, end_spine_index, _) = koreader_pos_to_calibre_cfi(
        koreader_highlight["pos1"], calibre_book_container
    )
...
def koreader_pos_to_calibre_cfi(koreader_pos, calibre_book_container):
    ...
    calibre_cfi = get_calibre_cfi_with_ids_and_offset(
        target_element, offset=koreader_offset
    )
...
# Generated by LLM ;)
def get_first_clean_descendant(element):
    current = element
    while True:
        children = [
...
# Generated by LLM ;)
def get_calibre_cfi_with_ids_and_offset(raw_element, offset):
    element = get_first_clean_descendant(raw_element) if offset == 0 else raw_element
    ...
    while element.getparent() is not None:
        ...
        siblings = [e for e in parent if isinstance(e.tag, str)]  # Only element nodes
        index = siblings.index(element)  # 0-based index
        cfi_step = 2 * (index + 1)  # 1-based index, multiplied by 2

        elem_id = element.get("id")
        step = f"{cfi_step}[{elem_id}]" if elem_id else str(cfi_step)
        steps.insert(0, step)
        element = parent

    # Handle root (start from the root element)
    siblings = (
        [e for e in element.getparent()]
    ...
    cfi_step = 2 * (index + 1)
    elem_id = element.get("id")
    root_step = f"{cfi_step}[{elem_id}]" if elem_id else str(cfi_step)
    steps.insert(0, root_step)

    # Append the character offset if provided
    if offset is not None:
        # When offset is 0, do not add the extra child element step
        if offset == 0:
            steps[-1] = f"{steps[-1]}/1:0"  # For the very first character, use :0
        else:
            steps[-1] = f"{steps[-1]}/1:{offset}"  # For other offsets, use /1:{offset}
